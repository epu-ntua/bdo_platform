...
def build_create_view_query(dataset1, dataset2):
    ...
    dataset1_vars = [c for c in dataset1_private_cols if is_variable(c)]
    dataset2_vars = [c for c in dataset2_private_cols if is_variable(c)]
...
def build_select_clause(common_columns, dataset1, dataset1_variables, dataset2, dataset2_variables):
    ...
    for c in common_columns:
        if c[0] == 'time':
            ...
            query += "round(%s.%s::NUMERIC, 1) AS %s, \n" % (dataset1, c[0], c[0])
        else:
            query += "%s.%s AS %s, \n" % (dataset1, c[0], c[0])
    idx = 0
...
def build_join_on_clause(common_columns, dataset1, dataset2):
    ...
    for c in common_columns:
        ...
        if c[0] == 'time':
            ...
            query += "round(%s.%s::NUMERIC, 1) = round(%s.%s::NUMERIC, 1) " % (dataset1, c[0], dataset2, c[0])
        else:
...
def build_group_by_clause(common_columns, dataset1):
    ...
    for c in common_columns:
        ...
        if c[0] == 'time':
            ...
            query += "round(%s.%s::NUMERIC, 1) " % (dataset1, c[0])
        if idx != length:
            query += ", \n"
    return query
...
def extract_dataset_id(dataset1):
    query = """SELECT id \n
            ...
            WHERE table_name = '%s' AND stored_at = 'UBITECH_POSTGRES' """ % dataset1
    cursor = connections['default'].cursor()
    # print query
    cursor.execute(query)
    res = cursor.fetchone()
